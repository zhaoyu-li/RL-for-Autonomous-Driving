...
# scenario_names = ["4lane_shanghai", "4lane_silverstone", "4lane_fakeinterlagos", "4lane_lala"]
...
for scenario_path, sv_num  in zip(scenario_paths, sv_nums):
    traffic = Traffic(
        flows=[
            Flow(route=RandomRoute(), begin=0,
                end=1 * 60 * 60,  # make sure end time is larger than the time of one episode
                rate=60,
    ...
    gen_traffic(
        scenario_path, traffic, name="all", output_dir=scenario_path, seed=seed, overwrite=True,
    )
